...
class MapTRHead(DETRHead):
    ...
    def loss_single(self,
        ...
        indexes = torch.sum(pts_weights, dim=[1, 2], keepdim=False)
        indexes = (indexes >= 40)
        pts_for_render_pred = pts_preds[indexes, :, :]
        cls_for_render_pred = cls_scores[indexes, :]

        pts_for_render_gt = normalize_2d_pts(unshifted_pts_targets, self.pc_range)[indexes, :, :]
        labels_for_render_gt = labels[indexes]

        isnotnan_for_rendering = torch.isfinite(pts_for_render_pred.reshape(pts_for_render_pred.shape[0], -1)).all(dim=-1)
        
        loss_rendered_mask = self.loss_rendered_mask(
            cls_for_render_pred[isnotnan_for_rendering],
            pts_for_render_pred[isnotnan_for_rendering], 
            labels_for_render_gt[isnotnan_for_rendering], 
            pts_for_render_gt[isnotnan_for_rendering]
        )
